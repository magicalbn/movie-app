...
searchMovies = async (query) => {
    try {
        const { data } = await axios.get(
            `${MOVIE_SEARCH_URL}/movie?api_key=${API_KEY}&language=en-US&page=1&include_adult=false&query=${query}`
        );
        return data;
    } catch (err) {
...
searchCast = async (query) => {
    try {
        const { data } = await axios.get(
            `${MOVIE_SEARCH_URL}/person?api_key=${API_KEY}&language=en-US&page=1&include_adult=false&query=${query}`
        );
        return data;
    } catch (err) {
        return err.response || err;
...
searchTV = async (query) => {
    try {
        const { data } = await axios.get(
            `${MOVIE_SEARCH_URL}/tv?api_key=${API_KEY}&language=en-US&page=1&include_adult=false&query=${query}`
        );
        return data;
    } catch (err) {
        return err.response || err;
...
router.get("/film/:query", (req, res) => {
    searchMovies(req.params.query).then((response) => {
        if (response.results) {
            ...
            res.status(403).send(
                response.data.status_message || "Not Able to Fetch"
            );
        }
    });
});
