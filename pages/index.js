...
class Index extends Component {
        ...
        if (searchQuery && searchQuery.trim() !== "") {
            switch (searchType) {
                case "Cast": {
                    ...
                    this.getSearchedMovieCast(searchQuery)
                    break;
                }
                case "TVshows": {
                    this.setState({ search: searchQuery, select: searchType })
    ...
    submitHandler = event => {
        ...
        else if (this.state.select == "Cast")
            this.getSearchedMovieCast(search)
    ...
    getSearchedMovieCast = (search) => {

        this.setState({ loading: true, headText: "cast" })
        searchCast(search).then(
            movies => this.setState({ movies, loading: false })
        ).catch(this.handleError)
    }
    ...
    render() {
        ...
        let titleText = "";
         switch (headText) {
            ...
            case "cast": { titleText= " Movies"; break;}
                ...
                {loading ? <Spinner animation="border" variant="primary" /> :
                    <>
                        ...
                        <Row>
                            {
                                movies.map(movie => {
                                    //   return <ListGroup.Item key={movie.id}> ID: <span className="link" >{movie.id}</span> Title: {movie.title} </ListGroup.Item>
                                    //return <Col xs={12} sm={6} md={6} lg={4} xl={3} className="display-card" key={movie.id}><img className="preview-image" onClick={() => Router.push(`/movie/?id=${movie.id}`)} src={movie.image_url}></img></Col>
